    ...
    const handleGenerateAI = async () => {
        ...
        try {
            ...
            await generateWithGeminiStream(
                prompt,
                (chunk) => {
                    // Her gelen metin parçası için mevcut metni güncelle
                    setStreamingResponse(prev => {
                        const updatedResponse = prev.diagnosis + chunk;

                        // Metinde bölümleri bul
                        const diagnosisMatch = updatedResponse.match(/Teşhis:(.*?)(?=Tedavi:|$)/s);
                        const treatmentMatch = updatedResponse.match(/Tedavi:(.*?)(?=Not:|$)/s);
                        const notesMatch = updatedResponse.match(/Not:(.*?)$/s);
